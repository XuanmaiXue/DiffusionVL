...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if hasattr(self.config, 'vision_tower_state_dict'):
            rank0_print(">>> Loading pre-converted Qwen vision tower weights...")
            incompatible_keys = vision_tower.vision_tower.load_state_dict(
                self.config.vision_tower_state_dict, strict=False
            )
        ...
        if hasattr(self.config, 'projector_state_dict'):
            rank0_print(">>> Loading pre-converted Qwen projector weights...")
            incompatible_keys = self.mm_projector.merger.load_state_dict(
                self.config.projector_state_dict, strict=False
            )
            rank0_print(f">>> Projector weight loading report: {incompatible_keys}")
            del self.config.projector_state_dict
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, image_grid_thw=None):
        ...
        if 'qwen' in name_lower:
            ...
            # Standard vision towers only need the image tensor
    ...
    def generate_conversation_ids(self, labels): # for LLaDA series models, adapted from LLaDA-V
        """
        Args:
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None, image_grid_thws=None, is_llada=False):
        vision_tower = self.get_vision_tower()

        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            # Add a check for is_llada, for llada model, we also need to get the conversation_ids
            if is_llada:
        ...
        for image in images:
            vision_tower = self.get_vision_tower()
            is_qwen = vision_tower and 'qwen' in vision_tower.vision_tower_name.lower()
            
            if is_qwen and image.dim() == 2:
        ...
        if 'qwen' in vision_tower.vision_tower_name.lower():
            # For Qwen-VL, the images are already processed features from the dataloader.
        ...
        if ordered_image_features_list is not None:
        ...
        # For Qwen-VL, the features are already processed, projected, and correctly ordered by its specific projector.
        if 'qwen' not in vision_tower.vision_tower_name.lower():
        ...
        if (self.training and 
            getattr(self.config, 'enable_bd3lm', False) and 
            getattr(self.config, 'bd3lm_block_aligned_eos', False)):
            block_size = getattr(self.get_model(), 'bd3lm_block_size', None)
            if block_size is not None:
                if is_llada:
                    eos_token_id = 126348
                else:
                    eos_token_id = getattr(self.config, 'eos_token_id', 151645)
                device = new_input_embeds.device
                batch_size, max_seq_len, hidden_size = new_input_embeds.shape
                             
                if attention_mask is not None and attention_mask.dim() == 2:
                    real_lengths = attention_mask.sum(dim=1)
                    
                    remainders = real_lengths % block_size
                    num_eos_per_sample = torch.where(
                        remainders > 0,
                        block_size - remainders,
                        torch.zeros_like(remainders)
                    )                    
                    padded_lengths = real_lengths + num_eos_per_sample
                    new_max_length = padded_lengths.max().item()
                    new_max_length = max(new_max_length, max_seq_len)
                    
                    positions = torch.arange(new_max_length, device=device).unsqueeze(0).expand(batch_size, -1)
                    
                    is_original = positions < real_lengths.unsqueeze(1)
                    is_eos = (positions >= real_lengths.unsqueeze(1)) & (positions < padded_lengths.unsqueeze(1))
                    
                    padded_inputs_embeds = torch.zeros(
                        batch_size, new_max_length, hidden_size,
                        dtype=new_input_embeds.dtype, device=device
                    )
                    padded_labels = torch.full(
                        (batch_size, new_max_length), IGNORE_INDEX,
                        dtype=new_labels.dtype, device=device
                    )
                    padded_attention_mask = torch.zeros(
                        (batch_size, new_max_length),
                        dtype=attention_mask.dtype, device=device
                    )
                    
                    copy_len = min(max_seq_len, new_max_length)
                    padded_inputs_embeds[:, :copy_len] = new_input_embeds[:, :copy_len]
                    padded_labels[:, :copy_len] = new_labels[:, :copy_len]
                    padded_attention_mask[:, :copy_len] = attention_mask[:, :copy_len]
                    
                    eos_embed = self.get_model().embed_tokens(
                        torch.tensor([eos_token_id], device=device)
                    ).squeeze(0)
                    
                    padded_inputs_embeds[is_eos] = eos_embed
                    padded_labels[is_eos] = eos_token_id
                    padded_attention_mask[is_eos] = True
                    ...
                    if position_ids is not None:
                        ...
                        if new_max_length > old_len:
                            additional_pos_ids = torch.arange(
                                old_len, new_max_length, 
                                dtype=position_ids.dtype, device=device
                            ).unsqueeze(0).expand(batch_size, -1)
                            position_ids = torch.cat([position_ids, additional_pos_ids], dim=1)
                else:
