...
class ModelArguments:
    ...
    enable_bd3lm: bool = field(
        default=False, 
        metadata={"help": "Enable Block Diffusion Language Model"}
    )
    
    bd3lm_block_size: int = field(
        default=4, 
        metadata={"help": "Block size for BD3-LM (1=AR, 4/8/16/BD3LM). When annealing is enabled, this is the target block size."}
    )
    
    bd3lm_block_aligned_eos: bool = field(
        default=True,
        metadata={"help": "padding eos to block boundary"}
    )
    ...
    # Qwen vision tower resolution control
    min_pixels: Optional[int] = field(default=384*384, metadata={"help": "Minimum number of pixels for Qwen vision tower"})
    max_pixels: Optional[int] = field(default=512*512, metadata={"help": "Maximum number of pixels for Qwen vision tower"})
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str, model_args: ModelArguments):
    """Collects the state dict and dump to disk."""
    
    if model_args.enable_bd3lm:
        bd3lm_params = {key: value for key, value in vars(model_args).items() if key.startswith("bd3lm_")}
        bd3lm_params["enable_bd3lm"] = model_args.enable_bd3lm
        
        for key, value in bd3lm_params.items():
            setattr(trainer.model.config, key, value)
        
        trainer.model.config.model_max_length = trainer.args.model_max_length
    
    if hasattr(model_args, 'min_pixels') and model_args.min_pixels is not None:
        trainer.model.config.min_pixels = model_args.min_pixels
        rank0_print(f"Saving min_pixels to config: {model_args.min_pixels}")
    if hasattr(model_args, 'max_pixels') and model_args.max_pixels is not None:
        trainer.model.config.max_pixels = model_args.max_pixels
        rank0_print(f"Saving max_pixels to config: {model_args.max_pixels}")
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are Qwen, created by Alibaba Cloud. You are a helpful assistant.") -> Dict:
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    """
    Preprocess conversation sources based on conversation template.
    Supports: qwen, llava_llada, llada_plain, plain
    """
    version = conversation_lib.default_conversation.version
    sep_style = conversation_lib.default_conversation.sep_style

    if sep_style == conversation_lib.SeparatorStyle.PLAIN or version == "llada_plain":
        return preprocess_plain(sources, tokenizer)
    if version == "llava_llada":
        ...
        try:
            ...
            if has_image:
                ...
                if hasattr(processor, 'crop_size'):
                    # CLIP/SigLIP style processor
                    crop_size = processor.crop_size
                    dummy_image = torch.zeros(3, crop_size["height"], crop_size["width"])
                    image_size = (crop_size["width"], crop_size["height"])
                    result["image"] = [
                        (dummy_image, image_size, "text"),
                    ]
                else:
                    min_size = 56
                    dummy_pil_image = Image.new('RGB', (min_size, min_size), color=0)
                    processor_output = processor.preprocess(dummy_pil_image, return_tensors="pt")
                    dummy_image_tensor = processor_output["pixel_values"]
                    
                    if dummy_image_tensor.dim() == 4:
                        dummy_image_tensor = dummy_image_tensor[0]
                    elif dummy_image_tensor.dim() == 3:
                        dummy_image_tensor = dummy_image_tensor[0]
                    
                    if "image_grid_thw" in processor_output:
                        if isinstance(processor_output["image_grid_thw"], torch.Tensor):
                            dummy_grid_thw = processor_output["image_grid_thw"][0].tolist()
                        else:
                            dummy_grid_thw = processor_output["image_grid_thw"]
                    else:
                        dummy_grid_thw = [1, 4, 4]
                    
                    image_size = (min_size, min_size)
                    result["image"] = [
                        (dummy_image_tensor, image_size, "text", dummy_grid_thw),
                    ]
...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        ...
        if "{" in data_path and "}" in data_path:
            ...
            with open(data_path, "r") as file:
                ...
                #     image_root: /path/to/images  # optional, for relative image paths
                ...
                for dataset in datasets:
                    ...
                    # Process image_root if provided
    ...
    def process_image(self, file, overwrite_image_aspect_ratio=None):
        image_folder = self.data_args.image_folder
        image_folder_2 = getattr(self.data_args, 'image_folder_2', None)
        processor = self.data_args.image_processor
        image_file = file  # Fix: use the parameter name correctly
        image_path = None
        
        # If image_file is already an absolute path (e.g., from YAML image_root processing), use it directly
        if os.path.isabs(image_file):
            ...
            # If no image_folder is provided and path is not absolute, try using image_file directly
            image_path = image_file
        ...
        # --- START OF MODIFICATION: Refactor to ensure processor_output is always assigned ---
        if image_aspect_ratio == "highres":
            image_tensor = process_highres_image(image, self.data_args.image_processor, self.data_args.image_grid_pinpoints)
            processor_output = {} # Create a dummy dict as grid_thw is not returned here
        elif image_aspect_ratio == "anyres" or "anyres_max" in image_aspect_ratio:
            image_tensor = process_anyres_image(image, self.data_args.image_processor, self.data_args.image_grid_pinpoints)
            processor_output = {} # Create a dummy dict as grid_thw is not returned here
        elif image_aspect_ratio == "crop_split":
            ...
            image = expand2square(image, tuple(int(x * 255) for x in processor.image_mean))
            processor_output = processor.preprocess(image, return_tensors="pt")
            image_tensor = processor_output["pixel_values"]
        else:
            processor_output = processor.preprocess(image, return_tensors="pt")
            image_tensor = processor_output["pixel_values"]
        
        # Handle the tensor shape compatibility for both CLIP-style and Qwen-style vision towers
        if image_tensor.dim() == 4:  # Standard CLIP/SigLIP vision tower output: [1, C, H, W]
            image_tensor = image_tensor[0] # remove batch dim -> [C, H, W]
        elif image_tensor.dim() == 3:  # Qwen style with batch dim: [1, num_patches, hidden_dim]
            image_tensor = image_tensor[0]
        elif image_tensor.dim() == 2:  # Qwen style without batch dim: [num_patches, hidden_dim]
            pass # Already in the correct shape
        else:
            raise ValueError(f"Unexpected pixel_values dimension: {image_tensor.dim()}")
        # --- END OF MODIFICATION ---
        
        grid_thw = None
        if "image_grid_thw" in processor_output:
            if isinstance(processor_output["image_grid_thw"], torch.Tensor):
                grid_thw = processor_output["image_grid_thw"][0].tolist()
            else:
                grid_thw = processor_output["image_grid_thw"]

        return image_tensor, image_size, "image", grid_thw
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "image" in instances[0]:
            original_images_data = [instance["image"] for instance in instances]

            image_tensors = []
            image_sizes = []
            modalities = []
            image_grid_thws = []
            has_grid_thw_data = False

            for im_list in original_images_data:
                for im_tuple in im_list:
                    image_tensors.append(im_tuple[0])
                    image_sizes.append(im_tuple[1])
                    modalities.append(im_tuple[2])
                    if len(im_tuple) > 3:
                        has_grid_thw_data = True
                        image_grid_thws.append(im_tuple[3])
                    else:
                        image_grid_thws.append(None)
            
            batch["images"] = image_tensors
            batch["image_sizes"] = image_sizes
            batch["modalities"] = modalities

            if has_grid_thw_data and any(g is not None for g in image_grid_thws):
                batch['image_grid_thws'] = image_grid_thws
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.enable_bd3lm:
        rank0_print("BD3-LM mode enabled!")
        rank0_print(f"   - Block size: {model_args.bd3lm_block_size}")

        if not (1 <= model_args.bd3lm_block_size <= training_args.model_max_length):
            raise ValueError(
                f"bd3lm_block_size ({model_args.bd3lm_block_size}) must be between 1 "
                f"and model_max_length ({training_args.model_max_length})."
            )
        if training_args.model_max_length % model_args.bd3lm_block_size != 0:
            raise ValueError(
                f"model_max_length ({training_args.model_max_length}) must be divisible "
                f"by bd3lm_block_size ({model_args.bd3lm_block_size})."
            )
        
        overwrite_config["enable_bd3lm"] = model_args.enable_bd3lm
        overwrite_config["bd3lm_block_size"] = model_args.bd3lm_block_size
        overwrite_config["bd3lm_block_aligned_eos"] = model_args.bd3lm_block_aligned_eos
        rank0_print(f"   - Block aligned EOS: {model_args.bd3lm_block_aligned_eos}")
        overwrite_config["bd3lm_sampling_eps_min"] = 1e-3
        overwrite_config["bd3lm_sampling_eps_max"] = 1.0
        overwrite_config["bd3lm_antithetic_sampling"] = True
        overwrite_config["model_max_length"] = training_args.model_max_length
    ...
    if model_args.model_class_name is not None:
        ...
        if "qwen" in model_args.model_name_or_path.lower():
            ...
            if model_args.force_model_type is not None:
            ...
            if is_diffusionvl_qwen3vl:
            ...
            elif is_diffusionvl_qwenvl:
                rank0_print("Loading DiffusionVL-QwenVL (Qwen2.5-VL + BD3-LM)")
            ...
            elif is_diffusionvl_qwen:
                rank0_print("Loading DiffusionVL-Qwen (Qwen2.5 + BD3-LM)")
                ...
                rank0_print("Loading standard LLaVA-Qwen (Autoregressive)")
            ...
            if model_args.force_model_type is not None:
                rank0_print(f"Using force_model_type: {model_args.force_model_type}")
                is_llada_bd3lm = model_args.force_model_type == "llava_llada_bd3lm"
            else:
                try:
                    ...
                    if os.path.exists(config_path):
                        ...
                        is_llada_bd3lm = model_type == "llada_bd3lm"
                    else:
                        is_llada_bd3lm = "bd3lm" in model_args.model_name_or_path.lower()
                except:
                    is_llada_bd3lm = "bd3lm" in model_args.model_name_or_path.lower()
            
            if is_llada_bd3lm:
                # LLaVA-LLaDA-BD3LM: LLaDA + BD3-LM
                from llava.model.language_model.llava_llada_bd3lm import LlavaLladaBD3LMConfig, LlavaLladaBD3LMForCausalLM

                rank0_print("Loading LLaVA-LLaDA-BD3LM (LLaDA + BD3-LM)")
                ...
                rank0_print("Loading standard LLaDA (full diffusion model)")
