#!/bin/bash
# Finetune script for DiffusionVL-QwenVL model on NPU (Ascend)
...
#   - torch_npu provides the Ascend backend (imported via TORCH_NPU_ENTRY)
...
# Model checkpoint path - Qwen2.5-VL model (converted to DiffusionVL format)
# TODO: Download Qwen2.5-VL-7B-Instruct from HuggingFace and convert to
#       DiffusionVL format, then set the path here.
PRETRAINED_CHECKPOINT="/path/to/Qwen2.5-VL-7B-Instruct-Reformat"
...
OUTPUT_DIR="./outputs/diffusionvl_qwenvl_finetune_npu"
...
custom_run_name=${3:-"diffusionvl_qwenvl_finetune_npu"}
...
echo "=========================================="
echo "DiffusionVL-QwenVL Finetune (Qwen2.5-VL + BD3-LM) on NPU"
echo "=========================================="
