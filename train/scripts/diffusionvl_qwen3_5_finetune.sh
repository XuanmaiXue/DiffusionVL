#!/bin/bash
# Finetune script for DiffusionVL-Qwen3.5 model
# Model type: diffusionvl_qwen3_5 (Qwen3.5 hybrid Gated DeltaNet + full attention + BD3-LM)
#
# Prerequisites:
#   - transformers >= 5.13 (ships Qwen3_5ForConditionalGeneration with hybrid support)
#   - A Qwen3.5 checkpoint converted to DiffusionVL format via
#     scripts/diffusionvl_prepare/convert_qwen3_5_to_diffusionvl.py
...
PRETRAINED_CHECKPOINT="/path/to/Qwen3.5-4B-DiffusionVL"

DATA_PATH="/path/to/your/training_data.json"
IMAGE_FOLDER="/path/to/your/images"
OUTPUT_DIR="./outputs/diffusionvl_qwen3_5_finetune"

# ============================================
num_node=$1
gpu_num=$2
custom_run_name=${3:-"diffusionvl_qwen3_5_finetune"}
...
echo "=========================================="
echo "DiffusionVL-Qwen3.5 Finetune (hybrid + BD3-LM)"
echo "=========================================="
...
torchrun --nproc_per_node=${gpu_num} --nnodes=${num_node} --master_addr=${MASTER_ADDR} --master_port ${MASTER_PORT} --node_rank=${RANK} \
    ...
    --mm_projector_type qwen3_5_merger \
    ...
    --weight_decay 0. \
    --warmup_ratio 0.03 \
    --force_model_type "diffusionvl_qwen3_5" \
